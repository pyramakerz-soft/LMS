...
<div class="container">
    ...
    <form action="{{ route('chapters.update', $chapter->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select name="unit_id" id="unit_id" class="form-control" required>
                @foreach ($units as $unit)
                    <option value="{{ $unit->id }}" {{ $chapter->unit_id == $unit->id ? 'selected' : '' }}>
                        {{ $unit->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="material_id" id="material_id" class="form-control" required>
                @foreach ($materials as $material)
                    <option value="{{ $material->id }}" {{ $chapter->material_id == $material->id ? 'selected' : '' }}>
                        {{ $material->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            @if ($chapter->image)
                <p>Current Image: <img src="{{ asset('storage/' . $chapter->image) }}" alt="{{ $chapter->title }}"
                        width="100"></p>
            @endif
        </div>
