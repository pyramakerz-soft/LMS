    ...
    <div class="main">
        ...
        <main class="content">
            <div class="container-fluid p-0">
                ...
                <form action="{{ route('observers.update', $observer->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control" id="name"
                            value="{{ $observer->name }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="gender" class="form-label">Gender</label>
                        <select name="gender" class="form-control" id="gender" required>
                            <option value="boy" {{ $observer->gender == 'boy' ? 'selected' : '' }}>Boy</option>
                            <option value="girl" {{ $observer->gender == 'girl' ? 'selected' : '' }}>Girl</option>
                        </select>
                    </div>
                    ...
                    <button type="submit" class="btn btn-primary">Update Observer</button>
                </form>
