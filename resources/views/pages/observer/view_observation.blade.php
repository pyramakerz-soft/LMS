    ...
    <div class="overflow-x-auto">
        <form action="{{ route('observation.store') }}" method="GET" enctype="multipart/form-data" class="mb-4 flex" style="gap:10px; padding:10px">
            ...
            <div class="questions mb-3" style="max-width: 70%;">
                ...
                <h3 class="text-base font-medium text-gray-700 mb-2" style="font-size:18px">- Provide an overall assessment of the teacher's effectiveness in teaching STEAM. Highlight strenghts, areas for improvement, and recommendations for professional development</h3>
                <textarea
                    name="note"
                ...
                <div class="info mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="school_id" id="school_id" class="w-full p-2 border border-gray-300 rounded" required>
                            <option value="{{$observation->school_id}}">{{App\Models\School::find($observation->school_id)->name}}</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="city" class="block text-sm font-medium text-gray-700">City</label>
                        <select name="city_id" id="city_id" class="w-full p-2 border border-gray-300 rounded" required>
                            <option value="" selected disabled>{{App\Models\School::find($observation->school_id)->city}}</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="grade" class="block text-sm font-medium text-gray-700">Grade</label>
                        <select name="grade_id" id="grade_id" class="w-full p-2 border border-gray-300 rounded" required>
                            <option value="{{$observation->stage_id}}" selected disabled>{{App\Models\Stage::find($observation->stage_id)->name}}</option>
                        </select>
                    </div>
                    <div class="mb-3">
